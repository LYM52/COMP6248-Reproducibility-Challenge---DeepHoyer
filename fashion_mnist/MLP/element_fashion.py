...
os.makedirs('saves_fashion', exist_ok=True)

# Training settings
parser = argparse.ArgumentParser(description='PyTorch FashionMNIST pruning from deep compression paper')
...
if args.pretrained:
    model.load_state_dict(torch.load('saves_fashion/elt_0.0_0.pth'))
    accuracy = test()
# Initial training
print("--- Initial training ---")
train(args.epochs, decay=args.decay, threshold=0.0)
accuracy = test()
torch.save(model.state_dict(), 'saves_fashion/elt_'+str(args.decay)+'_'+str(args.reg_type)+'.pth')
